...
class SetupTests(unittest.TestCase):
    ...
    def test_rerun_offers_saved_tv_address_and_preserves_other_settings(self):
        original = ("# Existing setup\nTV_HOST=existing-tv\n"
                    "FUGLERAMME_URL=http://birds.local:9090\n"
                    "TV_TOKEN_FILE=/custom/token.txt\nFRAME_SYNC_RETRIES=5\n")
        self.setup.CONFIG.write_text(original)
        with patch("builtins.input", return_value="") as prompt:
            self.setup.configure()
        self.assertIn("existing-tv", prompt.call_args.args[0])
        self.assertEqual(self.setup.CONFIG.read_text(), original)

    def test_rerun_changes_only_tv_address(self):
        self.setup.CONFIG.write_text("TV_HOST=old-tv\nFUGLERAMME_URL=http://birds:8080\n")
        with patch("builtins.input", return_value="new-tv"):
            self.setup.configure()
        self.assertEqual(self.setup.CONFIG.read_text(),
                         "TV_HOST=new-tv\nFUGLERAMME_URL=http://birds:8080\n")
    ...
    def test_invalid_addresses_are_reprompted_before_writing_config(self):
        with patch("builtins.input", side_effect=[
            "http://tv:8002", "frame.local",
        ]):
            self.setup.configure()
        self.assertIn("TV_HOST=frame.local\n", self.setup.CONFIG.read_text())
        self.assertIn("FUGLERAMME_URL=http://127.0.0.1:8080\n",
                      self.setup.CONFIG.read_text())

    def test_guided_install_does_not_request_existing_token_file(self):
        with patch.object(self.setup, "preflight"), \
                patch.object(self.setup, "install"), \
                patch.object(self.setup, "pair_and_test", return_value=True), \
                patch("builtins.input", side_effect=["frame.local"]):
    ...
    def test_empty_picture_confirmation_requires_an_answer(self):
        with patch.object(self.setup.subprocess, "run") as run:
            with patch("builtins.input", side_effect=["", "", "y"]) as prompt:
                self.assertTrue(self.setup.pair_and_test())
        self.assertEqual(sum("Is the Fugleramme collage" in call.args[0]
                             for call in prompt.call_args_list), 2)
        self.assertEqual(sum("start" in call.args[0]
