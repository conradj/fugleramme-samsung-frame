...
**Project update:** The note above describes the original setup. FrameSync now carries a matte to the next collage when the TV reports complete matte metadata for the currently displayed FrameSync image. Whether a matte can be chosen still depends on the image and TV model.
